...
describe('Coin', () => {
  it('different types for amount', () => {
    const ref = new Coin('uorb', 1000);
    const coins = [new Coin('uorb', 1000.0), new Coin('uorb', '1000')];
  ...
  it('deserializes Coin value', () => {
    ...
    expect(coin.denom).toEqual('uorb');
  ...
  it('arithmetic', () => {
    const zero = new Coin('uorb', 0);
    const coin = new Coin('uorb', 1000);
    const coin2 = new Coin('uorb', 2000);
    ...
    expect(sum.denom).toEqual('uorb');
    expect(coin.add(1500)).toEqual(new Coin('uorb', 2500));
    ...
    expect(diff.denom).toEqual('uorb');
    expect(diff.amount.toNumber()).toEqual(1000);
    expect(() => coin2.sub(coin3)).toThrow(Coin.ArithmeticError);

    // multiplication
    const product = coin.mul(3.1233);
    expect(product.denom).toEqual('uorb');
    expect(product.amount.toNumber()).toEqual(3123.3);

    // division
    const quotient = coin.div(5);
    expect(quotient.denom).toEqual('uorb');
    expect(quotient.amount.toNumber()).toEqual(200);

    // modulo
    const rem = coin.mod(43);
    expect(rem.denom).toEqual('uorb');
    expect(rem.amount.toNumber()).toEqual(coin.amount.toNumber() % 43);
  });

  it('equality', () => {
    const coin1 = new Coin('uorb', 1000);
    const coin2 = new Coin('uorb', 1000);
    const coin3 = new Coin('uorb', 1001);
    expect(coin1).toEqual(coin2);
    expect(coin1).not.toEqual(coin3);
  });

  it('toString', () => {
    const coin1 = new Coin('uorb', 123456);
    const coin2 = new Coin('uorb', 123456.789);
    expect(coin1.toString()).toEqual('123456uorb');
    expect(coin1.toDecCoin().toString()).toEqual('123456.0uorb');
    expect(coin2.toString()).toEqual('123456.789uorb');
  });

  describe('fromString', () => {
    it('parse IntCoin', () => {
      const coin1 = new Coin('uorb', 1001);
      const coin2 = Coin.fromString('1001uorb');
      expect(coin1).toEqual(coin2);

      const coin3 = new Coin('uorb', -1);
      const coin4 = Coin.fromString('-1uorb');
    ...
    it('parse DecCoin', () => {
      const coin1 = new Coin('uorb', 1001.5);
      const coin2 = Coin.fromString('1001.500000000000000000uorb');
      expect(coin1).toEqual(coin2);

      const coin3 = new Coin('uorb', '-1.0');
      const coin4 = Coin.fromString('-1.000000000000000000uorb');
      expect(coin3).toEqual(coin4);
    });
