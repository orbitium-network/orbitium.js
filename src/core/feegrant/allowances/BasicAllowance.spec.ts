...
describe('BasicAllowance', () => {
  it('amino type', () => {
    const ba = new BasicAllowance('1000uorb', now);
  ...
  it('both set', () => {
    ...
    expect(ba.toData().spend_limit).toEqual(new Coins('1000uorb').toData());
    expect(ba.toData().expiration).toEqual(now.toISOString());
    expect(ba.toProto().spendLimit).toEqual(new Coins('1000uorb').toProto());
    expect(ba.toProto().expiration).toEqual(now);
    expect(ba.toAmino().value.spend_limit).toEqual(
      new Coins('1000uorb').toAmino()
    );
    expect(ba.toAmino().value.expiration).toEqual(now.toISOString());
  });

  it('spend_limit only', () => {
    const ba = new BasicAllowance('1000uorb', undefined);
  ...
  it('spend_limit has zero amount', () => {
    expect(() => new BasicAllowance('1uorb,-1uusd', undefined)).toThrowError();
